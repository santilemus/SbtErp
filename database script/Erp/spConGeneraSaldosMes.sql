...
-- ================================================================================================
CREATE procedure [dbo].[spConGeneraSaldosMes] (
        @Empresa     int,
        @FechaHasta  datetime,
		@Usuario     varchar(25))
as
  ...
  begin try
    begin tran t1 
	  merge ConSaldoMes as destino
	    using (select s.Empresa, s.Periodo, s.Cuenta, @MesAnio, dbo.fnConSaldoCuentaOid(s.Empresa, @FechaDesde, s.Cuenta),
		              dbo.fnConCargosCuentaMes(@Empresa, s.Periodo, @iMes, s.Cuenta), dbo.fnConAbonosCuentaMes(@Empresa, s.Periodo, @iMes, s.Cuenta),
		  ...
		  when not matched by target then
		       insert (Empresa, Periodo, MesAnio, Cuenta, Mes, SaldoInicio, Debe, Haber, SaldoFin, UsuarioCrea, FechaCrea)
			   values (origen.Empresa, Origen.Periodo, Origen.MesAnio, origen.Cuenta, month(@FechaHasta), origen.SaldoInicioMes, 
			           origen.Debe, origen.Haber, origen.SaldoFin, @Usuario, current_timestamp)
...
EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'Generara los saldos mensuales por cuenta contable para EF y pivot table' , 
      @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'PROCEDURE',@level1name=N'spConGeneraSaldosMes'
GO
